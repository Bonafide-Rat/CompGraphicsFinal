...
class TerrainGeneration {
    ...
    CreateWall(position, rotationX, rotationY, noiseDivisor, floorSize) {
        ...
        const floorMat = new THREE.MeshPhongMaterial({side: THREE.DoubleSide, map: this.texture, normalMap: this.normalTexture});
    ...
    GenerateTriMesh(mesh, quaternion) {
        ...
        const terrainBody = new CANNON.Body({type: CANNON.Body.STATIC, shape: terrainTriMesh})
    ...
    CreateRoom(centrePoint, floorSize) {
        ...
        var wall1 = this.CreateWall(new THREE.Vector3(centrePoint.x - floorSize / 2, centrePoint.y, centrePoint.z), 0, Math.PI / 2, floorSize / 2, floorSize);

        var wall2 = this.CreateWall(new THREE.Vector3(centrePoint.x + floorSize / 2, centrePoint.y, centrePoint.z), 0, Math.PI / 2, floorSize / 2, floorSize);

        var wall3 = this.CreateWall(new THREE.Vector3(centrePoint.x, centrePoint.y, centrePoint.z - floorSize / 2), 0, 0, floorSize / 2, floorSize);

        var wall4 = this.CreateWall(new THREE.Vector3(centrePoint.x, centrePoint.y, centrePoint.z + floorSize / 2), 0, 0, floorSize / 2, floorSize);
